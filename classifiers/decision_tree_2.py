...
class Leaf_Node:
    ...
    def __init__(self, rows):
        x = list(class_counts(rows).keys())
        # print("h")
        # print(x)
        self.future_pred = x[0]
...
class DecisionTreeClassifier():
    ...
    def print_tree(self, node):
        if node is None:
            return
        if isinstance(node, Leaf_Node):
            print(node.future_pred)
            return
        self.print_tree(node.left_br)
        #print(node.rule)
        self.print_tree(node.right_br)
...
class SupportVectorMachine(object):
    ...
    def fit(self, X, y):
        ...
        self.bias = np.mean(
            [y_k - self.calculate_bias(x_k) for (y_k, x_k) in zip(self.support_vector_labels, self.support_vectors)])
